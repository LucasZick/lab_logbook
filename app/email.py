...
def send_password_reset_email(user):
    ...
    # Substitua pelo seu domínio real (com https:// e sem a barra no final)
    ...
    send_email('[Logbook] Recuperação de Senha',
               ...
               text_body=render_template('email/reset_password.txt',
                                         user=user, token=token, base_url=domain),
...
def send_invite_email(user, lab_name):
    ...
    send_email(f'[Logbook] Convite para participar do {lab_name}',
               sender=sender,
               recipients=[user.email],
               text_body=render_template('email/invite_professor.txt',
                                         user=user, lab_name=lab_name, token=token, base_url=domain),
